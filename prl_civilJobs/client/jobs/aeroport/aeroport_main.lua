...
Citizen.CreateThread(function()
    ...
    LoadModel(pedHash)
    local ped = CreatePed(2, GetHashKey(pedHash), zone.aero, Heading, 0, 0)
...
function StartTravailleJardinier()
    ...
    while AuTravailleJardinier do
        RageUI.Popup({
            message = "Un travaille t'as été attribué, dirige toi sur place !",
        })
        Wait(1)
            ...
            if count == random and AuTravailleJardinier then
                ...
                while not EnAction and AuTravailleJardinier do
                    ...
                    if dstToMarker <= 3.0 and AuTravailleJardinier then 
                      if GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), true)) ~= 3757070668 then
                          allow = true

                          allow = true
                          RageUI.Text({

                            message = "[~r~E~w~] Pour Travailler",
